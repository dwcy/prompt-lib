...
<!-- SPECKIT START -->
Active spec-kit feature: **012-headroom-tool** — Integrate Headroom (`chopratejas/headroom`, a context-compression layer for AI agents) into prompt-lib as a first-class managed tool in the cabal TUI: an installer module (Tools view, **MCP** group), an opt-in `headroom` MCP server template, and an investigate-only research spike on whether the proxy/wrap mode works with subscription/OAuth Claude Code. Out of scope: auto-compression nudge hook/skill, `headroom learn`. Planned 2026-06-21.
For technical context, structure, stack decisions, and constitution gate status, read [`specs/012-headroom-tool/plan.md`](specs/012-headroom-tool/plan.md). The full design tree is at `specs/012-headroom-tool/` (spec, plan, research, data-model, contracts, quickstart).
