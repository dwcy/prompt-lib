...
Also active: **020-env-variable-sources** — rename the Environment module to "Environment variables" and turn its two-pill Curated/System view into a multi-source, read-only browser for the selected project: one tab per discovered config file (`.env*`, `appsettings*.json`, `Properties/launchSettings.json`), plus Azure (Key Vault + app environments), Vercel, GitHub environments/variables/secrets, and .NET's out-of-repo developer secret store. Two rules drive the whole design: tabs load **names and metadata only** — a per-row eye fetches one value on demand and every reveal is audited — and **retrievability is a declared state**, because GitHub secrets and Vercel `sensitive` entries can never be read back and Key Vault is permission-gated. Integration is read-only against all three providers; the sole write is a per-project Azure link. Backend collectors follow the `dashboard_*_service.py` shape (never raise, `AvailabilityState` + hint) under `setup/src/cabal/`; frontend in `apps/cabal-desktop/src/modules/environment/`. Read [`specs/020-env-variable-sources/plan.md`](specs/020-env-variable-sources/plan.md); design tree at `specs/020-env-variable-sources/` (spec, plan, research, data-model, contracts, quickstart).
