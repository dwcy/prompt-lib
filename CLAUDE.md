...
<!-- SPECKIT START -->
Active spec-kit feature: **013-claude-dashboard** — Claude Session Dashboard: a Sessions screen in the Cabal Textual TUI showing all historical Claude Code sessions parsed from `~/.claude/projects/` JSONL transcripts. Shows token usage, estimated cost, agents dispatched with triggers, skills invoked, and raw log entries per session. Sessions can be deleted from disk. Planned 2026-06-30.
For technical context, structure, stack decisions, and constitution gate status, read [`specs/013-claude-dashboard/plan.md`](specs/013-claude-dashboard/plan.md). The full design tree is at `specs/013-claude-dashboard/` (spec, plan, research, data-model, quickstart).
