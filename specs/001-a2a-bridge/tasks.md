...
- [ ] T039 [P] Manual A2A Inspector pass for SC-002: follow `specs/001-a2a-bridge/quickstart.md` Step 9, point Inspector at both adapters, confirm zero Agent Card violations and zero message-flow violations, then record the result in `services/a2a-bridge/CHANGELOG.md` (create file if missing) — Owner: main. **DEFERRED — requires user to install the Inspector web tool (Python + Node frontend) and run interactively.**
