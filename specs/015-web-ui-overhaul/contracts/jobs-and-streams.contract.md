...
- States: `queued → running → succeeded | failed`; `queued|running → cancelled` through the registered `jobs.cancel` prepare/execute action (auth + effect-preview confirmation; cancellation is idempotent at the JobManager layer).
...
2. Preparing and executing `jobs.cancel` for a running fixture job yields `state: cancelled` on the stream AND in `GET /api/jobs/{id}`.
