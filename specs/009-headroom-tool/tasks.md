...
**Status**: ✅ Complete (4/4 — T009–T012) — install + MCP registration verified live (headroom 0.27.0 Connected); only an optional in-session compress demo remains as usage validation.
...
- [X] T011 Deploy via `python setup/settings-configurator-ui.py`, then run `quickstart.md` §1–§4 end-to-end: Tools install + status flip (SC-001/SC-002), MCP register + 3 tools in a fresh session + compress/retrieve round-trip (SC-003), opt-in absence check (SC-004), and regression of existing Tools/MCP entries (SC-006) — Owner: main *(VERIFIED: real `headroom_install()` auto-provisioned the toolchain and built from source → `headroom 0.27.0` on PATH [SC-001/002]; real `claude_mcp_add_from_template` registered it and `claude mcp list` shows `headroom … ✔ Connected` exposing the 3 tools [SC-003]; full 81-test suite passes [SC-006]. Deploy = N/A: the apply script doesn't propagate MCP.md/mcp-templates.json. Remaining as optional usage validation only: a live in-session `headroom_compress`→`headroom_retrieve` demo with `headroom proxy` running — usage, not feature wiring.)*
