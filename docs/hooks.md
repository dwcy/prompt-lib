...
**Why session boundaries, not mid-session**: Claude Code hooks are lifecycle-driven — there's no "CPU/RAM crossed a threshold" event. A hook can't watch a single long session slowly turning into syrup; it can only sweep what's left behind when a session starts or ends. Catching true mid-session buildup would need an OS-level timer (e.g. Windows Task Scheduler) running independently of Claude Code — deliberately out of scope here.
