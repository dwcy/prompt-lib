"""Integration tests for ProjectMcpScreen (T076).

Exercises the file primitives (_read_project_mcp, _write_project_mcp,
_template_to_entry) directly — no Textual driver, no async pilot. The screen's
...
def _make_screen(target_dir: pathlib.Path) -> ProjectMcpScreen:
    return ProjectMcpScreen(target_dir=target_dir)


def test_write_project_mcp_creates_file_with_correct_shape(tmp_project_dir):
    screen = _make_screen(tmp_project_dir)
    entries = {"foo": {"command": "echo", "args": ["hi"], "env": {}}}

    screen._write_project_mcp(entries)
...
def test_write_project_mcp_round_trip_via_enumerate(tmp_project_dir, monkeypatch):
    ...
    screen = _make_screen(tmp_project_dir)
    screen._write_project_mcp({"foo": {"command": "echo", "args": ["hi"], "env": {}}})
...
def test_write_project_mcp_cleans_temp_files(tmp_project_dir):
    ...
    screen._write_project_mcp({"x": {"command": "y", "args": [], "env": {}}})
...
def test_write_project_mcp_invalid_json_payload_raises(tmp_project_dir):
    ...
    with pytest.raises(TypeError):
        screen._write_project_mcp(bad)


@pytest.mark.parametrize("cmd", ["npx", "pnpm", "bunx"])
def test_template_to_entry_windows_wraps_pnpm_npx_bunx(tmp_project_dir, monkeypatch, cmd):
    monkeypatch.setattr("cabal.views.project_mcp.platform.system", lambda: "Windows")
    screen = _make_screen(tmp_project_dir)

    entry = screen._template_to_entry(
...
def test_template_to_entry_non_windows_passes_through(tmp_project_dir, monkeypatch):
    monkeypatch.setattr("cabal.views.project_mcp.platform.system", lambda: "Linux")
    screen = _make_screen(tmp_project_dir)
...
def test_template_to_entry_env_only_copies_set_vars(tmp_project_dir, monkeypatch):
    monkeypatch.setenv("FOO_TOKEN", "secret123")
