...
@pytest.mark.asyncio
async def test_update_panel_paints_cached_value_on_mount(isolated_cache):
...
async def test_update_panel_worker_writes_cache(isolated_cache, monkeypatch):
    ...
    monkeypatch.setattr(
        update_panel, "check_for_updates",
    ...
    async with app.run_test() as pilot:
        ...
        for _ in range(20):
            await pilot.pause()
            cached = widget_cache.load_entry("updates")
            if cached:
                break
...
_FAKE_ENV: dict = {
    "os": "Linux", "release": "6.1.0", "python": "3.11.1", "shell": "/bin/bash",
    "pkg_manager": "apt-get", "git": True, "git_user": "Tester", "git_version": "git version 2.45.0",
    "gh_login": None, "bash": True, "claude": True, "gh": True,
    "node": "v22.0.0", "npm": "10.0.0", "pnpm": None, "bun": None,
    "dotnet": None, "dotnet_sdks": [],
    "docker": None, "podman": None, "kubectl": None, "terraform": None,
    "az": None, "gcloud": None, "aws": None,
    "gemini": False, "codex": False, "opencode": False, "grok": False,
    "cursor": False, "windsurf": False, "copilot": False, "antigravity": False,
    "vscode": True, "rider": False, "visualstudio": False,
    "ollama": False, "ollama_models": [],
    "sqlcmd": False, "psql": False, "supabase": False, "neonctl": False,
    "target_exists": True,
}
