"""Codex A2A adapter app factory.

Wires the generic :class:`a2a_bridge.protocol.cli_runner.CliRunner` to the
Codex-specific NDJSON parser and command factory, exposes the standard
...
* ``tasks/get`` — returns the current state and accumulated artifacts for a
  ...
  unknown ids.
* ``tasks/cancel`` — terminates a running task and emits a final ``cancelled``
  SSE event; returns ``-32602`` with ``data.reason="task_already_terminal"``
...
should extract them into a shared helper now that a third adapter has landed.
"""
...
_AGENT_NAME = "codex-a2a-adapter"
_SKILL_ID = "codex-prompt"
...
def _build_card(host: str, port: int):
    return build_agent_card(
        ...
        host=host,
        port=port,
        description="A2A bridge adapter that delegates prompts to the OpenAI Codex CLI.",
        skills=[
            {
                "id": _SKILL_ID,
                "name": "codex-prompt",
                "description": "Forward a user prompt to the Codex CLI and stream its output.",
...
def _extract_prompt(params: dict[str, Any] | list[Any] | None) -> str:
    ...
    if not isinstance(task_payload, dict):
        raise JsonRpcError(ErrorCode.INVALID_PARAMS, "Invalid params", {"missing": ["task"]})
...
def _require_task(app: FastAPI, params: dict[str, Any] | list[Any] | None) -> Task:
    ...
    if not isinstance(task_id, str) or not task_id:
        raise JsonRpcError(ErrorCode.INVALID_PARAMS, "Invalid params", {"missing": ["task_id"]})
    task = app.state.tasks.get(task_id)
    if task is None:
        raise JsonRpcError(ErrorCode.INVALID_PARAMS, "Invalid params", {"reason": "task_not_found"})
    return task
...
def build_codex_app(
    *,
    bearer_token: str,
    cli_command_factory: Callable[[str], list[str]] = codex_command_factory,
    ...
    def handle_get(params: dict[str, Any] | list[Any] | None) -> dict[str, Any]:
        ...
        return {
            ...
            "last_state_change_at": task.last_state_change_at.isoformat().replace("+00:00", "Z"),
        }
