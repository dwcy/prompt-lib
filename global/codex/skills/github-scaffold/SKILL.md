...
3. **Read the user's stack hints** from `AGENTS.md` to pick a CI test command. If `AGENTS.md` doesn't exist, ask once: "What command runs your tests locally? (e.g. `pytest`, `pnpm test`, `dotnet test`, `python setup/tools/_smoketest.py`)"
...
Project-specific. Default skeleton — replace test command with whatever the user provided (or `AGENTS.md` documented):
...
- `@init-project` — calls `/github-scaffold` after writing `AGENTS.md` when the project has a GitHub remote.
