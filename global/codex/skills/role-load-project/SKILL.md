---
name: role-load-project
description: Role skill converted from Claude subagent. Loads an existing project's context by reading AGENTS.md, detecting the stack, and announcing which specialist subagents are available for this session. Run at the start of any existing project session.
...
Read `AGENTS.md` from the current directory. Extract:
...
Cross-reference with what AGENTS.md says. If they disagree, trust the file system and note the discrepancy.

## Step 3 — Summarize project context

Give the user a brief summary (3–5 bullet points) of the project based on AGENTS.md:
...
- Do not invent or assume conventions not found in AGENTS.md
- If AGENTS.md is missing key sections, note what's missing and suggest running @init-project to fill gaps
