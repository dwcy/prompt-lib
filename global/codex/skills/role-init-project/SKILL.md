---
name: role-init-project
description: Role skill converted from Claude subagent. Initializes a new project by detecting the stack, asking architecture questions, writing AGENTS.md, scaffolding a cross-platform `run` launcher (run / run.cmd / run.py with a random dev port), and spawning the correct specialist subagents for the detected language and project type.
...
Fill in the template's `## AGENTS.md Template` section with the user's answers and write it to `AGENTS.md` in the current directory.
...
After writing AGENTS.md, tell the user which specialist subagents are now available for this project type and what each one does:
...
- After writing AGENTS.md, summarize what was created.
