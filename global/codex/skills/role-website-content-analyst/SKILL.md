---
name: role-website-content-analyst
description: Role skill converted from Claude subagent. Web page research & extraction specialist. Use when you give one or more URLs (docs, articles, product pages, changelogs, blog posts) and want the content fetched, read, and distilled into structured findings — key facts, claims, quotes, links, and an assessment of relevance and credibility. Read-only on the web; writes a findings doc. Not for scanning code repositories (use @git-repo-analyst).
