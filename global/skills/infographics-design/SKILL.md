...
Count every feature from its **actual source files** — not what the README claims, not from memory. Use `scripts/audit_features.py` or run `Glob`/`Bash` equivalents. Cite the path in the cell footer so the infographic doubles as a lookup table (e.g. "16 agents · `global/agents/*.md`").
