...
| `## Slash commands` sub-tables | Every `[src](...)` link resolves (`global/skills/<name>.md`, `global/skills/<name>/SKILL.md`, `.claude/commands/<name>.md`, `.claude/skills/<name>.md`). Every `[docs](docs/<file>.md...)` link's file exists (anchors not checked). If a skill file exists on disk but no table row mentions it, propose adding a row. |
