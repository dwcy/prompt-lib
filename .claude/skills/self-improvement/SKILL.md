...
Stored at `.claude/skills/self-improvement/memory/`:
...
```bash
python .claude/skills/self-improvement/scripts/extract_lessons.py list [<file>]
# Lists all entry IDs + titles in lessons/mistakes/preferences/evals.

python .claude/skills/self-improvement/scripts/extract_lessons.py remove <file> <id>
# Removes a specific entry by ID. Prints the removed block to stderr so you can paste it back if needed.

python .claude/skills/self-improvement/scripts/extract_lessons.py validate
...
- **Storing what the code already shows.** If a future Claude can grep the repo and find the answer, don't store it.
