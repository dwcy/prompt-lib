...
For any task that implements a protocol surface (A2A endpoint, agent card, JSON-RPC method, REST contract, MCP tool schema):

- A `contract test` task for that surface MUST appear **before** the implementation task in this file.
- The contract test MUST exercise the wire format defined by the spec, not the in-process function signature.
- Contract test tasks live under `tests/contract/` (or the language equivalent path).
- This rule is binding only on protocol surfaces — unit tests remain optional.
...
- [ ] T002 Initialize [language] project with [framework] dependencies — Owner: @<lang>-architect
- [ ] T003 [P] Configure linting and formatting tools — Owner: main
...
- [ ] T010 [P] [US1] Contract test for [endpoint] in tests/contract/test_[name].py — Owner: @<lang>-tester
- [ ] T011 [P] [US1] Integration test for [user journey] in tests/integration/test_[name].py — Owner: @<lang>-tester
